...
* Before starting PS10: Re-read your notes from Lec24 and ModernDive 8.1 to make sure you understand all the definitions and notation. In particular:
    + We want to know the population proportion $p$ of some study population that is defined by researchers. We don't know this value and wish we did.
    + However, since a typical study population is very large, doing a full census to compute $p$ *exactly* is very expensive. So instead, we resort to random sampling to generate a random sample of $n$ observations which we use to compute a sample proportion $\widehat{p}$ that *estimates* $p$.
