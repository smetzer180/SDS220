...
# Population:
population_heights <- read_csv("https://goo.gl/BNmtVx")
View(population_heights)
population_heights %>% 
  summarize(sample_mean = mean(height))

# Painstakingly taken single sample of size n = 10
sampled_heights <- data_frame(
  name = c("Truc", "Isabel", "Roshni", "Jessie", "Caitlin", "Hannah", "Ojaswi", "Marcela", "Seri", "Nancy"),
  height = c(62, 62, 60, 68, 65, 63, 66, 67, 64, 64)
)
...
# Step 3.a): For the original sample, calculate the sample mean and save as x_bar
x_bar <- sampled_heights %>% 
  specify(response = height) %>% 
  calculate(stat = "mean")
x_bar

# Step 3.b): For each replicate, calculate the sample mean
sampled_heights %>% 
  specify(response = height) %>% 
  generate(reps = 1000) %>% 
  calculate(stat = "mean")
